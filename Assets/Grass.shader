...
Shader "Roystan/Grass"
{
    Properties
    {
		...
		_BladeCurved("Blade Curvature Amount", Range(1, 4)) = 2
	...
	void geo(triangle vertexOutput IN[3], inout TriangleStream<geometryOutput> triStream)
	{
		...
		float3 dis =  distance(_ObjectPos, worldPos);
		float3 circle = 1 - saturate(dis / _Radius);

		float3 sphereDisp = worldPos - _ObjectPos;
		sphereDisp *= circle;

		float3x3 facingRotationMatrix = AngleAxis3x3(rand(pos) * UNITY_TWO_PI, float3(0, 0, 1));

		float3x3 bendRotationMatrix = AngleAxis3x3(rand(pos.zzx) * _BendRotationRandom + (circle * UNITY_PI) * UNITY_PI * 0.5, float3(-1, 0, 0));
		...
		for (int i = 0; i < BLADE_SEGMENTS; i++)
		{
			...
			float segmentForward = pow(t, _BladeCurved) * forward;
